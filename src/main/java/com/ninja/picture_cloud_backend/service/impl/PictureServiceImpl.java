package com.ninja.picture_cloud_backend.service.impl;
...
import com.ninja.picture_cloud_backend.manager.FileManager;
import com.ninja.picture_cloud_backend.model.dto.file.UploadPictureResult;
...
import com.ninja.picture_cloud_backend.mapper.PictureMapper;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public Page<PictureVO> getPictureVOPage(Page<Picture> picturePage, HttpServletRequest request) {
        ...
        // 1. 关联查询用户信息
        //将图片中的所有userId封装到一个Set集合中
        Set<Long> userIdSet = pictureList.stream().map(Picture::getUserId).collect(Collectors.toSet());
        //根据userId查询用户列表，并且按照userId进行分组
        Map<Long, List<User>> userIdUserListMap = userService.listByIds(userIdSet).stream()
                .collect(Collectors.groupingBy(User::getId));

        System.out.println(userIdUserListMap);
        // 2. 填充信息
